...
class WIZnetTarget(Target):
    def __init__(self):
        Target.__init__(self)

    def init_hooks(self, hook, toolchain_name):
        hook.hook_add_binary("post", self.WIZnet_patch)

    @staticmethod
    def WIZnet_patch(t_self, resources, elf, binf):
        t_self.debug("WIZnet Patch: %s" % os.path.split(binf)[1])
        patch(binf)

class WIZwiki_W7500(WIZnetTarget):
    def __init__(self):
        WIZnetTarget.__init__(self)
        self.core = "Cortex-M0"
